...
parser.add_argument("--dataID", type=str, default="19",
                    help="The ID of the dataset in ../data/example[dataID] (default: 19)")

parser.add_argument("--stoneRep", type=str, default="available_stones_rot",
                    help="The folder name of the stone representation (default: available_stones_rot)")

parser.add_argument("--stoneIni", type=str, default="S",
                    help="The prefix of the stone file names (default: S)")


parser.add_argument("--wallDimX", type=int, default=70,
                    help="The dimension of the wall in X direction (default: 70)")

parser.add_argument("--wallDimY", type=int, default=70,
                    help="The dimension of the wall in Y direction (default: 70)")

parser.add_argument("--wallDimZ", type=int, default=40,
                    help="The dimension of the wall in Z direction (default: 40)")


parser.add_argument("--wallMaxX", type=int, default=None,
                    help="The maximum packing height in X direction (default: None)")

parser.add_argument("--nPose", type=int, default=1,
                    help="Number of candidate poses to evaluate per stone (default: 1)")


parser.add_argument("--seq", type=str, default="random",
                    help="The sequence of stone placement: random or volume (default: random)")
    ...
    for root, dirs, files in os.walk(data_dir):
        for file in files:
            if file.endswith('.obj') == False:
                continue
            stone_id = int(file.split(args.stoneIni)[1].split('.')[0])
...
def generate_from_placed_stones(layer_i,max_height,placed_stones_dir,available_stones_dir,world_size_meter,ground_mesh_dir,pitch = 1):
    ...
    angle_interval = step = 360 // args.nPose
    rotation_angle_options = [i * angle_interval for i in range(args.nPose)]#angles in degree
    nb_processor = 12
    nb_cand = 1#2
...
if __name__ == '__main__':
    layer_i = 1
    wallMaxX = args.wallMaxX if args.wallMaxX is not None else args.wallDimX
    max_height = args.wallDimX-wallMaxX+1
    data_dir = "../data/example"+f"{args.dataID}/"
    placed_stones_dir = "../data/example"+f"{args.dataID}/placed_stones_asbuilt/"
    available_stones_dir = "../data/example"+f"{args.dataID}/"+args.stoneRep+"/"

    wallDimX = args.wallDimX
    wallDimY = args.wallDimY
    wallDimZ = args.wallDimZ
    world_size_meter = (wallDimX+1,wallDimY+2,wallDimZ+2)
